...
def main(experiments_file, common_gridsearch, skip_error, random_state, n_cols):
    """
    Checks the rows of experiments_file in a loop. If there are no results in the row (pure fields after n_cols), 
    it takes the attributes from the columns and calls the script command until all result fields are filled.
    """
    if not random_state:
...
if __name__ == "__main__":
    ...
    experiments_file = 'etc/experiments.csv' # path to experiments table
